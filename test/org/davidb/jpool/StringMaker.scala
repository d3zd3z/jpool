/*
 * Generate pseudo random somewhat compressible strings.
 */
...
object StringMaker {
  /**
   * Make a new string with random words in it.  Returns strings that
   * start with the 'phase' in ascii (or at least a much as fits),
   * followed by a set of random english words randomly generated with
   * the given 'phase' as the random seed.  The same phase and length
   * arguments will generate the same resulting string.  Two strings
   * with the same phase and different lengths will have the same
   * initial characters as the shorter of the two strings. */
  def generate(phase: Int, length: Int): String = {
    val str = new StringBuilder(length + 10)
    val rnd = new Random(phase)
  ...
  // A list of words that we pick out.  This came from some random
  ...
  private val words =
      ("come get give go keep let make put seem take be do have say see " +
       ...
       "tired true violent waiting warm wet wide wise yellow young awake " +
